...
Disable evil-snipe-mode in the current buffer." nil nil)
...
If called interactively, toggle `Evil-Snipe-Local mode'.  If the
...
If called interactively, toggle `Evil-Snipe-Override-Local mode'.
...
With prefix ARG, enable Evil-Snipe mode if ARG is positive; otherwise, disable it.  If
called from Lisp, enable the mode if ARG is omitted or nil.

Evil-Snipe-Local mode is enabled in all buffers where `turn-on-evil-snipe-mode' would do
it.
...
With prefix ARG, enable Evil-Snipe-Override mode if ARG is positive; otherwise, disable
...
See `evil-snipe-override-local-mode' for more information on Evil-Snipe-Override-Local
mode.
