...
function loadipv4() {
	if ($('#ipv4').html()=='')
		chrome.tabs.query( {'active': true, currentWindow: true}, function(tabArray) {
			...
			$('#ipv4').load('http://www.tcpiputils.com/inc/tools.php?browseipv4='+getHostname(tabArray[0].url)+'&source='+source);
...
function loadipv6() {
	if ($('#ipv6').html()=='')
		chrome.tabs.query( {'active': true, currentWindow: true}, function(tabArray) {
			...
			$('#ipv6').load('http://www.tcpiputils.com/inc/tools.php?browseipv6='+getHostname(tabArray[0].url)+'&source='+source);
...
function loadisp() {
	if ($('#isp').html()=='')
		chrome.tabs.query( {'active': true, currentWindow: true}, function(tabArray) {
			...
			$('#isp').load('http://www.tcpiputils.com/inc/tools.php?browseas='+getHostname(tabArray[0].url)+'&source='+source);
...
function loaddomain() {
	if ($('#domain').html()=='')
		chrome.tabs.query( {'active': true, currentWindow: true}, function(tabArray) {
			...
			$('#domain').load('http://www.tcpiputils.com/inc/tools.php?browsedomain='+getHostname(tabArray[0].url)+'&source='+source);
...
function loadmyip() {
	if ($('#myip').html()=='') {
		...
		$('#myip').load('http://www.tcpiputils.com/inc/tools.php?myip=1&source='+source);
		_gaq.push(['_trackEvent', 'ip-address', 'myip', 'chromeext']);
	}
}
function restore_options() {
  if (navigator.vendor.substring(0,5).toLowerCase()=="opera") source="operaext";
  else source="chromeext";
