...
enyo.kind({
	...
	*/
    ...
    this.dragT0 = new Date().getTime(); this.flickx = this.x, this.flicky = this.y;this.dragging = !0, this.my = a.pageY, this.py = this.uy = this.y, this.mx = a.pageX, this.px = this.ux = this.x;
    }, 
    drag: function(a) {
        if(this.dragging) {
            var b = this.vertical ? a.pageY - this.my : 0; this.uy = b + this.py, this.uy = this.boundaryDamping(this.uy, this.topBoundary, this.bottomBoundary, this.kDragDamping);
            ...
            if(currentsign != this.lastsign) {
                this.dragT0 = new Date().getTime(); this.flickx = this.x, this.flicky = this.y;
            }
    ...
    dragDrop: function(a) {
        if(this.dragging && !window.PalmSystem) {
            var b = .5; this.y = this.uy, this.y0 = this.y - (this.y - this.y0) * b, this.x = this.ux, this.x0 = this.x - (this.x - this.x0) * b;
        }
        this.dragging = !1;
    }, 
    dragFinish: function() {
